...
class FourRoomsMini(MiniWorldEnv):
    """
    ...
    def _gen_world(self):
        ...
        #     min_x=-3.5, max_x=-1, min_z=1, max_z=3.5, wall_tex="cardboard"
        # )
        room0 = self.add_rect_room(
            min_x=-4.2, max_x=-0.6, min_z=0.6, max_z=4.2, wall_tex="cardboard"
        )
        # Top-right room
        room1 = self.add_rect_room(
            min_x=0.6, max_x=4.2, min_z=0.6, max_z=4.2, wall_tex="marble"
        )
        # Bottom-right room
        room2 = self.add_rect_room(
            min_x=0.6, max_x=4.2, min_z=-4.2, max_z=-0.6, wall_tex="metal_grill"
        )
        # Bottom-left room
        room3 = self.add_rect_room(
            min_x=-4.2, max_x=-0.6, min_z=-4.2, max_z=-0.6, wall_tex="stucco"
        )

        # Add openings to connect the rooms together
        self.connect_rooms(room0, room1, min_z=1.8, max_z=3, max_y=2.2)
        self.connect_rooms(room1, room2, min_x=1.8, max_x=3, max_y=2.2)
        self.connect_rooms(room2, room3, min_z=-3, max_z=-1.8, max_y=2.2)
        self.connect_rooms(room3, room0, min_x=-3, max_x=-1.8, max_y=2.2)

        self.box_bottomRight = self.place_entity(
            Box(color="green"), pos=np.array([1.15, 0.0, 1.15]), dir=0
        )
        self.box_topLeft = self.place_entity(
            Box(color="yellow"), pos=np.array([-1.15, 0.0, -1.15]), dir=0
        )
        ...
        # print room sizes
        for room in self.rooms:
            print(room.max_x - room.min_x, room.max_z - room.min_z)
...
class FourRoomsMiniActions(FourRoomsMini):
    """
...
class FourRoomsMiniTwoTasks(FourRoomsMiniActions):
...
class FourRoomsMiniTask1(FourRoomsMiniTwoTasks):
    def __init__(self):
        super().__init__(task_id=0)


class FourRoomsMiniTask2(FourRoomsMiniTwoTasks):
    def __init__(self):

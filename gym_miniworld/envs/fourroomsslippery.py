...
class FourRoomsSlippery(MiniWorldEnv):
    """
    Classic four rooms environment but with slippery actions in the top right and bottom left rooms.
    ...
    def _gen_world(self):
        ...
        room0 = self.add_rect_room(
            min_x=-7, max_x=-1, min_z=1, max_z=7, wall_tex="cardboard"
        )
        # Top-right room
        room1 = self.add_rect_room(
            min_x=1, max_x=7, min_z=1, max_z=7, wall_tex="marble"
        )
        # Bottom-right room
        room2 = self.add_rect_room(
            min_x=1, max_x=7, min_z=-7, max_z=-1, wall_tex="metal_grill"
        )
        # Bottom-left room
        room3 = self.add_rect_room(
            min_x=-7, max_x=-1, min_z=-7, max_z=-1, wall_tex="stucco"
        )
        ...
        self.box_bottomRight = self.place_entity(
            Box(color="green"), pos=np.array([1.5, 0.0, 1.5]), dir=0
        )
        self.box_topLeft = self.place_entity(
            Box(color="yellow"), pos=np.array([-1.5, 0.0, -1.5]), dir=0
        )
...
class FourRoomsSlipperyActions(FourRoomsSlippery):
    """
    ...
    def step(self, action):
        ...
        # If agent is in the top right or bottom left rooms, then with slippery probability,
        # the action will be replaced by a random action.
        # This is to simulate the slippery actions in the top right and bottom left rooms.
        # room 0 is the bottom left room and room 2 is the top right room
        if self.agent.room in [0, 2]:
            if np.random.uniform() < self._slippery_prob:
                while True:
                    new_rand_action = self.action_space.sample()
                    if new_rand_action != action:
                        action = new_rand_action
                        break
    ...
    def step_with_agent_pos_dir(self, action):
        ...
        agent_in_room_idx = self.agent.room
...
class FourRoomsSlipperyTwoTasks(FourRoomsSlipperyActions):
    def __init__(self, task_id: int = 0, slippery_prob: float = 0.15):
        super().__init__(
            max_episode_steps=4000, coverage_plot=False, slippery_prob=slippery_prob
        )
...
class FourRoomsSlipperyTask1(FourRoomsSlipperyTwoTasks):
    def __init__(self):
        super().__init__(task_id=0)


class FourRoomsSlipperyTask2(FourRoomsSlipperyTwoTasks):
    def __init__(self):
        super().__init__(task_id=1)


class FourRoomsSlippery30Task1(FourRoomsSlipperyTwoTasks):
    def __init__(self):
        super().__init__(task_id=0, slippery_prob=0.3)


class FourRoomsSlippery30Task2(FourRoomsSlipperyTwoTasks):
    def __init__(self):
        super().__init__(task_id=1, slippery_prob=0.3)


class FourRoomsSlippery45Task1(FourRoomsSlipperyTwoTasks):
    def __init__(self):
        super().__init__(task_id=0, slippery_prob=0.45)


class FourRoomsSlippery45Task2(FourRoomsSlipperyTwoTasks):
    def __init__(self):
        super().__init__(task_id=1, slippery_prob=0.45)


class FourRoomsSlippery60Task1(FourRoomsSlipperyTwoTasks):
    def __init__(self):
        super().__init__(task_id=0, slippery_prob=0.60)


class FourRoomsSlippery60Task2(FourRoomsSlipperyTwoTasks):
    def __init__(self):
        super().__init__(task_id=1, slippery_prob=0.60)

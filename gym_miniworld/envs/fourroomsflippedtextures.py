...
class FourRoomsFlippedTextures(MiniWorldEnv):
    """
    ...
    def _gen_world(self):
        ...
        room0 = self.add_rect_room(
            min_x=-7, max_x=-1, min_z=1, max_z=7, wall_tex="floor_tiles_bw", floor_tex="cardboard"
        )
        # Top-right room
        room1 = self.add_rect_room(
            min_x=1, max_x=7, min_z=1, max_z=7, wall_tex="floor_tiles_bw", floor_tex="marble"
        )
        # Bottom-right room
        room2 = self.add_rect_room(
            min_x=1, max_x=7, min_z=-7, max_z=-1, wall_tex="floor_tiles_bw", floor_tex="metal_grill"
        )
        # Bottom-left room
        room3 = self.add_rect_room(
            min_x=-7, max_x=-1, min_z=-7, max_z=-1, wall_tex="floor_tiles_bw", floor_tex="stucco"
        )
...
class FourRoomsFlippedTexturesActions(FourRoomsFlippedTextures):
    """
    Classic four rooms environment.
    The agent must reach the red box to get a reward.
    """
...
class FourRoomsFlippedTexturesTwoTasks(FourRoomsFlippedTexturesActions):
    def __init__(self, task_id: int = 0):
        super().__init__(max_episode_steps=4000, coverage_plot=False)
...
class FourRoomsFlippedTexturesTask1(FourRoomsFlippedTexturesTwoTasks):
    def __init__(self):
        super().__init__(task_id=0)


class FourRoomsFlippedTexturesTask2(FourRoomsFlippedTexturesTwoTasks):
    def __init__(self):

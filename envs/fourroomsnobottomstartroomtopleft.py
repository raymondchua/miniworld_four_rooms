...
class FourRoomsNoBottomStartRoomTopLeft(MiniWorldEnv):
    ...
    def _gen_world(self):
        ...
        self.connect_rooms(room1, room2, min_x=3 * self._scale, max_x=5 * self._scale, max_y=2.2)
        self.connect_rooms(room2, room3, min_z=-5 * self._scale, max_z=-3 * self._scale, max_y=2.2)
        self.connect_rooms(room3, room0, min_x=-5 * self._scale, max_x=-3 * self._scale, max_y=2.2)

        self.box_bottomRight = self.place_entity(
            Box(color="green"), pos=np.array([1.5 * self._scale, 0.0, 1.5 * self._scale]), dir=0
        )
        self.box_topLeft = self.place_entity(
            Box(color="yellow"), pos=np.array([-1.5 * self._scale, 0.0, -1.5 * self._scale]), dir=0
        )
        ...
        self.place_agent(dir=agent_dir_radians, pos=self.agent_pos)
    ...
    def _gen_world_with_agent_pos_dir(
        ...
        if agent_pos is not None:
            _, valid_position, room_idx_assigned = self.place_agent_with_valid_position_checks(
                dir=agent_dir_radians, pos=agent_pos
            )

        elif agent_pos is None and room_idx is not None:
            ent, valid_position, room_idx_assigned = self.place_agent_randomly_in_specific_room(
        ...
        # if both agent_pos and room_idx are None, then randomly place the agent
            ...
            ent, valid_position, room_idx_assigned = self.place_agent_randomly_with_valid_position_checks(
...
class FourRoomsNoBottomStartRoomTopLeftActions(FourRoomsNoBottomStartRoomTopLeft):
    """
...
class FourRoomsNoBottomStartRoomTopLeftTwoTasks(FourRoomsNoBottomStartRoomTopLeftActions):
    ...
    def step_with_agent_pos_dir(self, action):
        ...
        # if self.agent.dir_degrees is not one of the 0, 90, 180, 270, then round it to the nearest one
        if self.agent.dir_degrees not in [0, 90, 180, 270]:
            self.agent.dir_degrees = int((round(self.agent.dir_degrees / 90.0)) * 90) % 360
...
class FourRoomsNoBottomStartRoomTopLeftTask1(FourRoomsNoBottomStartRoomTopLeftTwoTasks):
    def __init__(self):
        super().__init__(task_id=0)


class FourRoomsNoBottomStartRoomTopLeftTask2(FourRoomsNoBottomStartRoomTopLeftTwoTasks):
    def __init__(self):
        super().__init__(task_id=1)


class FourRoomsNoBottomStartRoomTopLeftSize2Task1(FourRoomsNoBottomStartRoomTopLeftTwoTasks):
    def __init__(self):
        super().__init__(task_id=0, scale=1.2)


class FourRoomsNoBottomStartRoomTopLeftSize2Task2(FourRoomsNoBottomStartRoomTopLeftTwoTasks):
    def __init__(self):
        super().__init__(task_id=1, scale=1.2)


class FourRoomsNoBottomStartRoomTopLeftSize3Task1(FourRoomsNoBottomStartRoomTopLeftTwoTasks):
    def __init__(self):
        super().__init__(task_id=0, scale=1.5)


class FourRoomsNoBottomStartRoomTopLeftSize3Task2(FourRoomsNoBottomStartRoomTopLeftTwoTasks):
    def __init__(self):
